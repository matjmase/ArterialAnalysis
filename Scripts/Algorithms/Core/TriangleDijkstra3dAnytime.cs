...
public class TriangleDijkstra3dAnytime : IIteratableProcess
{
    ...
    private HashSet<PositionCoord3d> _scaledNeighborsHash;

    protected PositionCoord3d[] _neighbors => _scaledNeighbors;
    protected HashSet<PositionCoord3d> _neighborsHash => _scaledNeighborsHash;
    ...
    public TriangleDijkstra3dAnytime()
    {
    }
    ...
    public void Initialize(DijkstraProcessModel model)
    {
        ...
        _scaledNeighbors = _cubeNeighbors.Select(e => e * scaler).OrderBy(e => PositionCoord3dFloat.Magnitude(e.ToFloat())).ToArray();
        _scaledNeighborsHash = _scaledNeighbors.ToHashSet();
    ...
    public void Iterate()
    {
        ...
        Func<PositionCoord3d, bool> failCondition = coord => _archivedPoints.Contains(coord) || _aquisitionPointsHash.ContainsKey(coord) || !_isValidNeighbor(focus.Coord, coord);
        for (var i = 0; i < _neighbors.Length && newlyAquiredPoints.Count != 2; i++)
        {
            var newCoord = focus.Coord + _neighbors[i];

            if(newlyAquiredPoints.Count == 0 && failCondition(newCoord))
            {
                continue;
            }
            else if(newlyAquiredPoints.Count != 0 && failCondition(newCoord) && !_neighborsHash.Contains(newlyAquiredPoints.First().Coord - newCoord))
            {
                continue;
            }
